...
func testNewCore(backend istanbul.Backend, config *istanbul.Config, state State, current *roundState, valSet istanbul.ValidatorSet) *core {
	c := &core{
		config:             config,
...
func NewTestSystemWithBackend(n, f uint64) *testSystem {
	...
	for i := uint64(0); i < n; i++ {
		...
		core := testNewCore(backend, config, StateAcceptRequest, newRoundState(&istanbul.View{
			Round:    big.NewInt(0),
			Sequence: big.NewInt(1),
		}, vset, newTestPreprepare(&istanbul.View{
			Round:    big.NewInt(0),
			Sequence: big.NewInt(1),
		}, nil, backend.address).(*qbfttypes.Preprepare), nil, nil, nil, func(hash common.Hash) bool {
			return false
		}), vset)
