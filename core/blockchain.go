...
// QUORUM
// checks if the consensus engine is Raft
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, psManager mps.PrivateStateRepository, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		log.Info("track: triedb.Reference")
		triedb.Reference(root, common.Hash{}) // metadata reference to keep trie alive
		bc.triegc.Push(root, -int64(block.NumberU64()))
		log.Info("track: triedb.Reference completed")
	...
	if !reorg && externTd.Cmp(localTd) == 0 {
		log.Info("track: !reorg && externTd.Cmp(localTd) == 0")
	...
	if status == CanonStatTy {
		log.Info("track: writeHeadBlock")
		bc.writeHeadBlock(block)
		log.Info("track: writeHeadBlock completed")
	}
	bc.futureBlocks.Remove(block.Hash())

	if status == CanonStatTy {
		log.Info("track: bc.chainFeed.Send")
		bc.chainFeed.Send(ChainEvent{Block: block, Hash: block.Hash(), Logs: logs})
		log.Info("track: bc.chainFeed.Send completed")
		if len(logs) > 0 {
		...
		// event here.
		if emitHeadEvent {
			log.Info("track: 1773 chainHeadFeed")
...
func (bc *BlockChain) InsertChainWithoutSealVerification(block *types.Block) (int, error) {
	...
	bc.wg.Add(1)
	bc.chainmu.Lock()
	n, err := bc.insertChain(types.Blocks([]*types.Block{block}))
	bc.chainmu.Unlock()
	bc.wg.Done()
